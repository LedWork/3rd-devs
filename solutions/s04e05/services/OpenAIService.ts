...
export class OpenAIService {
  ...
  async preprocessImage(imagePath: string): Promise<Buffer> {
    try {
      const image = await sharp(path.join(__dirname, 'storage/', imagePath));
      
      // Process the image with various enhancements
  ...
  async ocrImage(imagePath: string): Promise<ImageProcessingResult> {
    try {
      // Preprocess the image first
      const processedImage = await this.preprocessImage(imagePath);
      await fs.writeFile(`${imagePath}_processed.jpg`, processedImage);
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "Please transcribe this Polish handwritten text accurately. Maintain all punctuation and line breaks as they appear in the original text. If you're unsure about any words, do your best to transcribe them."
          },
          {
            role: "user",
            content: [
      ...
      console.error(`Error OCRing image ${imagePath}:`, error);
      throw error;
    }
  }

  async ocrImages(imagePaths: string[]): Promise<ImageProcessingResult[]> {
    try {
      const results = await Promise.all(imagePaths.map(path => this.ocrImage(path)));
      return results;
    } catch (error) {
      console.error("Error OCRing multiple images:", error);
      throw error;
    }
  }
