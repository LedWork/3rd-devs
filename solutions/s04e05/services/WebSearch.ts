...
export class WebSearchService {
  ...
  async crawlUrls(urls: string[], conversation_uuid: string): Promise<WebContent[]> {
    console.log('Input (crawlUrls):', urls);
    ...
    const scrapePromises = scrappableUrls.map(async (url) => {
      try {
        ...
        // Add retry mechanism
        const crawlWithRetry = async (retryCount = 0): Promise<any> => {
          try {
            return await this.firecrawlApp.crawlUrl(url, {
              limit: 2,
              maxDepth: 2,
              scrapeOptions: {
            ...
            if (retryCount < 3) { // Max 3 retries
              console.log(`Rate limit hit for ${url}, waiting 60 seconds before retry...`);
              await new Promise(resolve => setTimeout(resolve, 60000)); // Wait 1 minute
              return crawlWithRetry(retryCount + 1);
            }
            throw error;
          }
        };

        const scrapeResult = await crawlWithRetry();
    ...
    const scrapePromises = scrappableUrls.map(async (url) => {
      try {
        ...
        if (scrapeResult && scrapeResult.markdown) {
          return { url, content: scrapeResult.markdown.trim(), links: scrapeResult.links };
        } else {
          console.warn(`No markdown content found for URL: ${url}`);
          return { url, content: '', links: [] };
        }
